...
(function (root, factory) {
  ...
  const MOE_RECIPES = [
    ...
    },
    {
      ...
      task: 'Prepare the next execution step for exactly one opportunity from {validation}, subject to {constraints}. First verify from the supplied result that validation_status is exactly VALIDATED and that this status is supported by the predeclared authoritative observations and threshold evaluation. If the status is UNTESTED, INCONCLUSIVE, FALSIFIED, missing, internally inconsistent, or unsupported by the recorded evidence, return launch_state BLOCKED_NOT_VALIDATED and do not promote the opportunity. Preserve the original opportunity identity, hypothesis, evidence references, observation timestamps, measured validation cost and unknown fields unchanged; do not rewrite estimates into facts. Build the smallest viable launch package needed for the specific model: define the offer or product, target buyer, channel, positioning, fulfillment path, pricing hypothesis, unit-economics model, dependencies, compliance or platform constraints, measurable launch objective, authoritative result sources, explicit success/failure criteria, maximum time and money at risk, and stop conditions. Clearly separate OBSERVED FACTS, ESTIMATES, ASSUMPTIONS and DECISIONS. Where a numeric input is unsupported, keep it unknown and show which decision depends on learning it. Reuse StarNet existing planning, capability resolution, consent, files and orchestration paths; do not create or mirror a separate approval, budget, transaction, inventory, order, customer or revenue truth source inside this workflow. Prepare only actions that are actually reachable through granted tools. Any consequential action such as publishing, outreach, purchasing, ordering, payment, ad spend, subscription, contractual acceptance or other financial commitment must pass the canonical capability and consent path at execution time; if no canonical permitted path exists, mark that action BLOCKED_CAPABILITY rather than simulating it. Produce an ordered launch plan that distinguishes reversible preparation from irreversible or externally visible actions, names the exact preflight checks before each consequential action, and defines idempotency or duplicate-prevention requirements wherever retries could create duplicate listings, messages, orders or charges. After building the plan, classify launch_state deterministically: READY_TO_EXECUTE only when validation is proven, every mandatory launch input and exposure bound is present, every required action has a canonical reachable tool path, and required preflight plus duplicate-prevention checks are defined; BLOCKED_CAPABILITY when a required action has no canonical permitted tool path; BLOCKED_PREFLIGHT when a mandatory launch input, exposure bound, preflight requirement or duplicate-prevention requirement is unresolved. Never use READY_TO_EXECUTE merely because a plan or asset was generated. For every ordered action in a READY_TO_EXECUTE handoff, assign a stable action_id and record the intended side effect, canonical capability/tool identity, whether fresh consent is required, the maximum exposure delta, preflight assertions, idempotency or duplicate-prevention strategy, the dependency action_ids that must resolve before it may run, and the authoritative reconciliation source that can prove what actually happened. A downstream executor must treat this handoff as immutable input: if the action parameters, exposure bounds, target, price, quantity, recipient, platform, dependencies or required capability change, it must return to planning instead of silently widening the approved action. Capability availability and consent must be re-resolved immediately before each consequential side effect; readiness is not a cached permission. Execute an action only when every declared dependency action_id has a reconciled result that permits the dependent action to proceed. If a required dependency is BLOCKED_AT_EXECUTION, DENIED_AT_EXECUTION or OUTCOME_UNKNOWN, do not execute its dependent consequential actions. If execution later returns an ambiguous or unknown outcome, do not retry a consequential action until the authoritative reconciliation source has established whether the first attempt took effect. Require the downstream executor to emit one execution_result per action_id using only EXECUTED_CONFIRMED, NOT_APPLIED_CONFIRMED, BLOCKED_AT_EXECUTION, DENIED_AT_EXECUTION or OUTCOME_UNKNOWN. EXECUTED_CONFIRMED and NOT_APPLIED_CONFIRMED require evidence from the declared authoritative reconciliation source plus an observed timestamp; provider acknowledgements, task completion, local intent logs or agent assertions alone are insufficient. OUTCOME_UNKNOWN must freeze automatic retry and any dependent consequential action until reconciliation resolves it. Execution must be restart-safe: after any worker or process restart, rebuild execution eligibility from the immutable handoff plus authoritative reconciliation results, never from a local resume cursor, completed-action cache, task checkpoint or worker memory. Before treating an action without a confirmed terminal result as runnable after restart, reconcile its stable action_id and idempotency reference against the declared authoritative reconciliation source. If that source confirms the side effect applied, emit EXECUTED_CONFIRMED and never replay it; if it confirms the side effect did not apply, make the action eligible again only after fresh capability, consent and preflight checks; if it cannot determine the outcome, emit OUTCOME_UNKNOWN and freeze that action and its dependents. Any runnable_action_ids or resume position must be derived transiently from reconciled results and dependencies and must not be persisted as canonical execution truth. Derive terminal launch_execution_state only from the reconciled execution_result set: EXECUTION_CONFIRMED only when every required action is EXECUTED_CONFIRMED or an explicitly acceptable NOT_APPLIED_CONFIRMED; PARTIAL_EXECUTION when at least one required side effect is EXECUTED_CONFIRMED but another required action is BLOCKED_AT_EXECUTION or DENIED_AT_EXECUTION; EXECUTION_BLOCKED when no required side effect was confirmed and a required action is BLOCKED_AT_EXECUTION or DENIED_AT_EXECUTION; OUTCOME_UNKNOWN when any required action remains OUTCOME_UNKNOWN. Never collapse OUTCOME_UNKNOWN into success or failure. Aggregate launch execution only from these per-action results: never mark the launch completed, failed, spent, ordered, published or charged from planned actions alone. Preserve provider transaction, order, listing or message identifiers when the authoritative source returns them, but do not fabricate identifiers when absent. Do not infer launch success from task completion, generated assets, agent confidence or projected economics. End with one structured handoff containing launch_state, launch_execution_state when execution results exist, unchanged validation evidence, launch package, ordered actions, stable action_ids, dependency action_ids, required capabilities, consent checkpoints, authoritative measurement and reconciliation sources, exposure bounds, stop conditions, unresolved unknowns and the single next action for the existing planning/orchestration path. Do not automatically scale after launch; scaling requires a later decision based on real measured outcomes.',
