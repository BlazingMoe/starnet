...
const out='.worldshots/vibrant-lighting';mkdirSync(out,{recursive:true});let proc,cdp;
try {
  ...
  const original=execFileSync('git',['show','28a9ed488:frontend/app/stationbake.js'],{encoding:'utf8',maxBuffer:4*1024*1024});
  ...
  assert.ok(result.after.chroma>result.before.chroma*1.05,'material colour should measurably recover');
  assert.ok(result.after.luma<result.before.luma*1.25,'colour recovery must not become an exposure surge');
