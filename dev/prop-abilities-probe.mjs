...
try{
  ...
  await check('Selected purpose, scope and access are visible outside About',`!document.querySelector('.refit-propworkspace').classList.contains('show-details')&&['.refit-preview-grant','.refit-purpose','.equipment-status','.refit-equipment-scope','[data-preview-place]','[data-preview-cancel]'].every(s=>document.querySelector(s).getBoundingClientRect().height>0)`);
  await check('All five core choices and the Place control fit in the initial desktop view',`(()=>{const g=document.querySelector('#refit-propgrid-host').getBoundingClientRect(),d=document.querySelector('.refit-dock').getBoundingClientRect();return [...document.querySelectorAll('.refit-core-card')].every(b=>{const r=b.getBoundingClientRect();return r.top>=g.top&&r.bottom<=g.bottom+1})&&document.querySelector('[data-preview-place]').getBoundingClientRect().bottom<=d.bottom})()`);
  ...
  await check('Changing agent refreshes the visible access explanation',`document.querySelector('.equipment-status').textContent.includes(window.__otherName)&&document.querySelector('.refit-ability-overview select').value!==window.__originalAgent`);
  ...
  await run(`document.querySelector('#refit-fit').click();document.querySelector('[data-preview-place]').click();window.__before=window.__fixture.props().length`);
  ...
  await run(`document.body.style.zoom='';document.body.style.removeProperty('--sn-unzoom');document.querySelector('[data-preview-cancel]').click()`);await check('Cancel safely returns to Select',`Build.isOpen()&&Build.__test__.tool()==='select'`);
