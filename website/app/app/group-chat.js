...
const GroupChat = (() => {
  ...
  function name(id) { return id === 'user' ? 'YOU' : (roster.find(a => a.id === id)?.name || id); }
  function init() {
    if (root) return;
    const bar = $('comms-idbar'); if (!bar) return;
    const launch = h('div', { id: 'gc-launch', class: 'gc-launch' });
    launch.append(button('GROUP CHAT', () => picker(false)), button('ADD AGENTS', () => picker(true)));
    bar.before(launch);
    ...
    tools.append(button('PARTICIPANTS', () => picker(true)), button('PAUSE', () => action('pause')), button('RESUME', () => action('resume')));
    const more = h('details'); more.append(h('summary', {}, 'Group options'));
    ...
    sendOptions.append(button('SEND', () => sendText(input.value)), button('ASK EVERYONE', () => sendText(input.value, { all: true })), button('INTERRUPT & SEND', () => sendText(input.value, { interrupt: true })), independent, summary);
    composer.append(recipient, h('div', { id: 'gc-mentions', role: 'listbox', 'aria-label': 'Mention participants' }), input, sendOptions);
    root.append(header, tools, more, pins, files, transcript, states, composer, h('div', { id: 'gc-notice', role: 'status' }));
    ...
    const css = h('style'); css.textContent = `
      ...
      .gc-picker{position:fixed;inset:12% 15%;z-index:50000;overflow:auto;padding:20px;background:var(--bg,#101514);color:var(--ph,#8fc9ac);border:1px solid currentColor;box-shadow:0 12px 80px #000}
  ...
  function paint() {
    ...
    $('gc-header').textContent = group.members.map(name).join(' · ') + ' | Lead: ' + name(group.leadId) + (group.paused ? ' | PAUSED' : '');
    ...
    if (costs.length) states.append(h('div', {}, 'Measured usage: $' + costs.reduce((s, t) => s + t.usd, 0).toFixed(4) + (costs.length < group.turns.filter(t => t.endedAt).length ? ' · some costs unavailable' : '')));
  ...
  async function picker(convert) {
    ...
    const dialog = h('section', { id: 'gc-picker', class: 'gc-picker', role: 'dialog', 'aria-modal': 'true', 'aria-label': existing ? 'Edit participants' : 'Create group chat' });
    dialog.append(h('h3', {}, existing ? 'PARTICIPANTS' : 'CREATE GROUP CHAT'));
    const title = h('input', { type: 'text', maxlength: '80', 'aria-label': 'Group title', value: existing?.title || origin?.title || 'Group chat' }); dialog.append(title);
    const checks = [];
    for (const a of roster) {
      ...
      const label = h('label'); label.append(box, document.createTextNode(' ' + a.name + ' [' + a.id + '] · ' + (a.model || 'model not configured'))); dialog.append(label); checks.push(box);
    }
    const lead = h('select', { 'aria-label': 'Group lead' });
    function options() { const previous = lead.value; lead.replaceChildren(); for (const c of checks.filter(c => c.checked)) lead.append(h('option', { value: c.value }, name(c.value))); if ([...lead.options].some(o => o.value === previous)) lead.value = previous; }
    for (const c of checks) c.addEventListener('change', options); options(); if (existing) lead.value = existing.leadId;
    dialog.append(h('label', {}, 'Default responder'), lead, h('p', {}, 'Participants receive this session’s relevant history, instructions and shared files when they run. Adding someone does not start a run.'));
    const errors = h('p', { role: 'alert' }); dialog.append(errors);
    dialog.append(button(existing ? 'SAVE PARTICIPANTS' : 'CREATE', async () => {
      try {
    ...
    if (!existing && info.templates.length) {
      dialog.append(h('h4', {}, 'SAVED GROUPS'));
      for (const t of info.templates) dialog.append(button(t.title, async () => { const g = await api({ op: 'create', ...t, id: uid() }); adopt(g); save(); dialog.remove(); App.openWorkstream(g.id); }));
    }
    dialog.addEventListener('keydown', e => {
      if (e.key === 'Escape') dialog.remove();
      if (e.key === 'Tab') { const nodes = [...dialog.querySelectorAll('input,select,button')]; const first = nodes[0], last = nodes.at(-1); if (e.shiftKey && document.activeElement === first) { e.preventDefault(); last.focus(); } else if (!e.shiftKey && document.activeElement === last) { e.preventDefault(); first.focus(); } }
    });
    document.body.append(dialog); title.focus();
  }
